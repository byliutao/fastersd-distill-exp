...
class CLIPCapDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        c_data = tokenizer(self.prefix + c_data).squeeze()
...
def main():
    ...
    with open(args.candidates_json) as f:
        if os.path.splitext(args.candidates_json)[-1] == '.json':
            candidates = json.load(f)
        elif os.path.splitext(args.candidates_json)[-1] == '.csv':
            import pandas as pd
            df = pd.read_csv(args.candidates_json)
            candidates = {}
            for _, row in df.iterrows():
                case_number = str(row.case_number)
                candidates[case_number] = str(row.prompt)
        elif os.path.splitext(args.candidates_json)[-1] == '.npz':
            npz_file = np.load(args.candidates_json, allow_pickle=True)
            captions = npz_file['captions'][()]
            candidates = {}
            for case_number, caption in enumerate(captions):
                hash_value = get_hash_value(caption)
                candidates[hash_value] = caption
        else:
            raise ValueError('Unsupported file form')
...
# coco-val2014
# ---------------------------------------------------------------------------------------------------------------------#
# SwiftBrush reproduce with 30K subset captions of coco-val2014
# python clipscore.py ../val2014_captions.npz ../results/val2014
# CLIPScore:
#
